...
setwd("C:/Users/adria/OneDrive/Dokumenter/5. Semester/SOK-2008/Innlevering 3")
...
ledighet <- read_excel("ledighet.xlsx", skip = 5, col_names = FALSE,
                       n_max = 3)

colnames(ledighet) <- c("Alder", "Utdanningsnivå", "Arbeidsledighet")

ledighet <- ledighet[-2,-2]

ggplot(ledighet, aes(Alder, Arbeidsledighet)) +
  geom_bar(stat = "identity", color = "black", fill = "coral3") +
  geom_text(aes(label = Arbeidsledighet),
            vjust = 2, color = "white", size = 5) +
  ylim(0, 15) +
  labs(title = "3.1 - Arbeidsledighet i forskjellige aldersgrupper",
       x = "Aldersgrupper",
       y = "Arbeidsledighet (i %)") +
  theme_light()

# Kommentar i Word-dokument
